...
# This branch keeps blocked factorization experimental rather than including it
# in the package. Loading it here gives CI a correctness/performance A/B before
# any public dispatch is changed.
...
function _bitwise_same(A, B)
    size(A) == size(B) || return false
    return all(i -> A[i] === B[i], eachindex(A, B))
end
...
function bench_potrf_x4(n)
    ...
    blocks = Dict{Int,Matrix{Float64x4}}()
...
function check_potrf_remainder()
    ...
    U = copy(A)
    B = copy(A)
    MFLA._potrf_unblocked_kernel!(U)
    MFLA._potrf_blocked_vec8!(B, Val(16))
    @assert _bitwise_same(B, U)
    @assert all(MultiFloats.isnormalized, B)
end
