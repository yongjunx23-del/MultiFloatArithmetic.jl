...
- maximum observed relative error over the fixed corpus;
- `-log2(max relative error)` as an observed effective-bit indicator.

This metric includes both input representation error and arithmetic error, which
is appropriate for a user-level precision comparison. It is empirical, not a
